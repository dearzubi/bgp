...
def main():
    os.system("rm -f /tmp/R*.log /tmp/R*.pid /tmp/R*.api logs/*")
    ...
    for router in net.switches:
        if router.name == ROGUE_AS_NAME and not FLAGS_rogue_as:
            continue
	#router.cmd("mkdir /tmp/%s" % (router.name))
        router.cmd("/usr/sbin/zebra -f conf/zebra-%s.conf -d -z /tmp/zserv-%s.api -i /tmp/zebra-%s.pid > logs/%s-zebra-stdout 2>&1" % (router.name, router.name, router.name, router.name))
        router.waitOutput()
        router.cmd("/usr/sbin/bgpd -f conf/bgpd-%s.conf -d -z /tmp/zserv-%s.api -i /tmp/bgp-%s.pid > logs/%s-bgpd-stdout 2>&1" % (router.name,router.name, router.name, router.name), shell=True)
        router.waitOutput()
        log("Starting zebra and bgpd on %s" % router.name)

    for host in net.hosts:
        host.cmd("ifconfig %s-eth0 %s" % (host.name, getIP(host.name)))
        log("%s-eth0 ip %s" % (host.name, getIP(host.name)))
